...
## Known Limitations (Resolved in Phase 3)

1. **Database Integration:**
   - ~~Currently using mock data for users~~ (Resolved: Fully integrated with Medusa Customer/User tables in Phase 3)
   - ~~Need to implement User model in Phase 3~~ (Resolved)
   - ~~Auth endpoints will be updated with real DB queries~~ (Resolved)

2. **Token Storage:**
   - ~~Refresh tokens not yet stored in Redis~~ (Resolved: Full Redis module integration completed in Phase 3)
   - ~~Cannot revoke tokens server-side yet~~ (Resolved: Logout now revokes Redis tokens)
   - Relying on token expiration for security (Enhanced with strict Redis validation)

3. **Email Notifications:**
   - ~~Email verification tokens generated but not sent~~ (Resolved)
   - ~~Password reset emails not implemented~~ (Resolved)
   - ~~Will be added with email service integration~~ (Resolved: `emailService` created for dev logging & SendGrid prep)
